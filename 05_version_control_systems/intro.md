...
| オプション/使用例 | 説明 |
| --- | --- |
| `git push origin [ブランチ名]:[リモートブランチ名]` | [リモートブランチ名]に、[ブランチ名]の内容を反映させる。[リモートブランチ名]がリモートリポジトリに存在しない場合は新規作成される。ちなみに、「現在地がmasterブランチの時の`git push origin master`」と「`git push origin master:master`」は同義。|
...
- 状態には「open」「close」「Draft」の三種類あり、管理者が「いやこれはよくないな」と思ったらcloseされる。
...
- オリジナルリポジトリをFork 👉 コピーリポジトリ作成 👉 ローカルリポジトリにclone（手元に落とす）👉 作業　という流れ。
- 更新して欲しい時は、コピーリポジトリからオリジナルのリポジトリにPRを出す。
- ForkしたいリポジトリのForkボタンを押せば自分のリポジトリにコピーが作成されるという便利仕様。Forkしたら作者に通知行くから気をつけてね。
...
最初はGitHubからリモートリポジトリをローカルにcloneするところから始まる。そこで新しくブランチを切り、何かしらの作業をして、それをリモートリポジトリにコミットするところからスタート。

1. コミット完了後、リモートリポジトリにpushする。
2. githubに行く。pushしたブランチがある。

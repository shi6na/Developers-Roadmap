...
- Gitを利用したサービス。
- GUIによるコードの閲覧・管理はもちろん、ソースコードをダウンロードできたり、直接コメントつけることも可能。コードメインのSNS。
- `PullRequest(PR)`という、「コードのレビューをお願いします、あわよくばマージしてください申請」機能がある。
- 弊社はForkするタイプのPullRequest方式をよく使うそう。確かに研修でもそれを使った。
- マスコットキャラクターはOctocatという種族の「Monalisa（モナリサ）」ちゃん。
...
- 以下、一例。Template：「Basic」のデフォルトカラム。
  ...
  - In progress(作業中)
  - Done(完了)
- そこに項目を増やしていき、作業中や完了など、進捗ごとにカラムを動かして使う。夏休みの宿題の進捗管理これだったら絶対上手くやってた・・・

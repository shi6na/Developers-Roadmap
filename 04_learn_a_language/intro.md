...
- 処理系が色々用意されていて、それによってnodeとかECMAScriptを切り替える。
...
- 複数スレッドを用いた並列処理も可能。ECMAScriptではスレッドが定義されていない（Webページは基本シングルスレッド）ので使えないが、webworkerなどを使うと実装できるようになる。

## Ruby

## Go

## 規約とコーディングスタイルは別々に考える
